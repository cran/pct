...
## ---- echo=FALSE, warning=FALSE, fig.show='hold', fig.cap="Top 5 LSOA-LSOA desire lines with highest number of people cycling (left) and driving (right) in the Isle of Wight."----
# at the lsoa level
...
# cor(l_original$dutch_slc / l_original$all, l_msoa$pcycle_dutch)
# cor(l_original$govtarget_slc / l_original$all, l_msoa$pcycle_govtarget)
# plot(l_original$dutch_slc / l_original$all, l_msoa$pcycle_dutch)
...
leaflet() %>% 
  addTiles() %>% 
  addPolylines(data = rnet_walk, weight = rnet_walk$foot / mean(rnet_walk$foot) * 5)
